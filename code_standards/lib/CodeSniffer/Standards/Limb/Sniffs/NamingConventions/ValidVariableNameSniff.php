 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Limb_Sniffs_NamingConventions_ValidVariableNameSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
        ...
        if ($tokens[$objOperator]['code'] === T_OBJECT_OPERATOR) {
            ...
            if ($tokens[$var]['code'] === T_STRING) {
                ...
                if ($tokens[$bracket]['code'] !== T_OPEN_PARENTHESIS) {
                    ...
                    if (false === self::isUnderScores($objVarName)) {
                        $error = "Variable \"$originalVarName\" is not in valid under_scores format";
    ...
    protected function processMemberVar(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (false === self::isUnderScores($varName)) {
            $error = "Variable \"$varName\" is not in valid under_scores format";
            $phpcsFile->addError($error, $stackPtr);
        } else if (preg_match('|\d|', $varName)) {
    ...
    static function isUnderScores($string)
    {
        $loweredString = strtolower($string);
        return $loweredString == $string;
    }
